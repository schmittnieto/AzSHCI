...
.DESCRIPTION
    ...
    - Adds the converted images to the Azure Stack HCI library with cleaned-up names and correct generation.
...
$hcipassword = "dgemsc#utquMHDHp3M"  # Replace with your actual password or handle securely
...
function Import-Images {
    ...
    foreach ($ImageSKU in $ImagesToDownload) {
        ...
        # Clean up Image Name for Azure Stack HCI
        $ImageName = $DiskName -replace '[^a-zA-Z0-9]', ''
        Write-Message "Prepared Image Name: $ImageName" -Type "Info"

        $OSType = if ($LatestImage.OsDiskImage.OperatingSystem -eq "Windows") { "Windows" } else { "Linux" }
        ...
        Write-Message "Adding image $ImageName to Azure Stack HCI library..." -Type "Info"
        try {
            ...
            Write-Message "Image $ImageName added to Azure Stack HCI library successfully." -Type "Success"
        } catch {
            Write-Message "Failed to add image $ImageName to Azure Stack HCI library. Error: $_" -Type "Error"
            Invoke-Command -VMName $nodeName -Credential $hciCredentials -ScriptBlock {
                param($LibraryVolumeName, $DiskName)
                Remove-Item -Path "C:\ClusterStorage\$LibraryVolumeName\$DiskName.vhdx" -Force -ErrorAction SilentlyContinue
            } -ArgumentList $LibraryVolumeName, $DiskName
