...
# Automates the process of pulling VM images from Azure and adding them as custom images to Azure Stack HCI using Bicep.

<#
.SYNOPSIS
    Downloads selected VM images from Azure and adds them as custom images to Azure Stack HCI using Bicep.
...
.DESCRIPTION
    ...
    - Adds the converted images to the Azure Stack HCI library using a Bicep template.
...
$BicepFile = Join-Path -Path $TemplateDirectory -ChildPath "template.bicep"
...
function Import-Images {
    ...
    foreach ($ImageSKU in $ImagesToDownload) {
        ...
        # Define Bicep template content with required properties
        $bicepTemplate = @"
...
resource galleryImage 'Microsoft.AzureStackHCI/galleryImages@2023-09-01-preview' = {
  name: imageName
  location: location
  extendedLocation: {
...
"@
        ...
        # Avoid reserved names in publisher, offer, and sku
        $publisherName = $ImageSKU.PublisherName -replace '[^a-zA-Z0-9]', ''
        $offerName = $ImageSKU.Offer -replace '[^a-zA-Z0-9]', ''
        $skuName = $ImageSKU.Skus -replace '[^a-zA-Z0-9]', ''
        ...
        # Write Bicep template and parameters to files
        try {
            ...
            $parametersFileContent | ConvertTo-Json -Depth 10 | Out-File -FilePath $ParameterFile -Encoding utf8 -Force
            Write-Message "Bicep template and parameters files created at $BicepFile and $ParameterFile." -Type "Success"
        } catch {
            Write-Message "Failed to write Bicep template or parameters files. Error: $_" -Type "Error"
            continue
        }

        # Deploy the Bicep template
        Write-Message "Deploying Bicep template to create image $ImageName..." -Type "Info"
        try {
            $deploymentName = "CreateImage-$ImageName"
            New-AzResourceGroupDeployment -Name $deploymentName -ResourceGroupName $ResourceGroupName -TemplateFile $BicepFile -TemplateParameterFile $ParameterFile -Verbose -ErrorAction Stop | Out-Null

            Write-Message "Image $ImageName added to Azure Stack HCI library successfully using Bicep template." -Type "Success"

            # Remove template and parameter files after successful deployment
            try {
                Remove-Item -Path $BicepFile -Force
                Remove-Item -Path $ParameterFile -Force
                Write-Message "Removed Bicep template and parameters files." -Type "Success"
            } catch {
                Write-Message "Failed to remove Bicep template or parameters files. Error: $_" -Type "Warning"
            }
        } catch {
            Write-Message "Failed to deploy Bicep template for image $ImageName. Error: $_" -Type "Error"
            continue
        }
    }
}
